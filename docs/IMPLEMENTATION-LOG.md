# Log Implementasi Sistem Presensi SMA Negeri 4 Lhokseumawe

File ini adalah log kronologis per-task (T00–T31) dari Sistem Presensi PWA untuk SMA Negeri 4 Lhokseumawe — berisi keputusan arsitektur/teknis, dependensi migration, kriteria verifikasi, batas implementasi, dan status progres. Dokumen ini **bukan** panduan pengguna; untuk deskripsi produk, cara menjalankan, dan cara mengakses dari perangkat lain, lihat [README.md](../README.md).
...
## Aksesibilitas dan kontras warna (audit WCAG 2.1)

Audit kontras terhadap design system T31 dijalankan pada 2026-09-26 untuk tiga role (Siswa, Guru, Admin) dan perbaikannya masuk ke `app/static/css/tailwind.input.css`:

- Token `--color-danger` dipergelap `#b54444` → `#9c3232` dan `--color-warning` `#9b6515` → `#8a5610` agar badge Alpa/Terlambat masing-masing 6.0:1 dan 5.7:1 pada latar badge (semula 4.51:1 dan 4.54:1, tanpa margin aman).
- Penanda non-teks `.attendance-dot` (Lampiran B, kalender) kini memakai ring `1px rgba(48,48,68,.65)`; isian tint pale sebelumnya hanya 1.7:1 terhadap cream, di bawah ambang 3:1 WCAG 1.4.11.
- `.eyebrow` dinaikkan dari 12px ke 13px dengan `line-height: 1.4` agar label uppercase tidak kecil.
- `.data-table` diberi `line-height: 1.55` untuk keterbacaan tabel dense (audit log, monitoring).
- Border kontrol form tetap memakai `--color-control-border` (3.97:1) dan bukan `--color-border` dekoratif (1.24:1); `aria-label` tombol ikon sudah lengkap di ketiga role sehingga tidak ada tombol ikon tanpa label.

Regresi dikunci di `tests/test_ui.py` (`test_shared_text_and_status_colors_meet_contrast_targets` dan `test_non_text_status_markers_have_visible_edge_and_labels_stay_readable`) sehingga perubahan token di masa depan harus tetap memenuhi ambang. Semua badge status sudah memakai label teks (bukan hanya warna) dan kalender selalu menyertakan `aria-label` per tanggal.
...
T31 ditutup pada tahap Opsi A dan Opsi B (lihat [Audit visual T31](../docs/UX-VISUAL-AUDIT-T31.md) untuk bukti lengkap). Ringkasan Opsi A: token `--color-danger` diperkuat ke `#9c3232` (6.0:1) dan `--color-warning` ke `#8a5610` (5.7:1); penanda non-teks seperti dot kalender mendapat ring 1px agar lolos WCAG 1.4.11; label uppercase dinaikkan ke 13px; baris tabel diberi `line-height` 1.55. Regresi dikunci di `tests/test_ui.py`.

## Audit visual T31 — Opsi B (PWA shell dan polish)

Opsi B menutup celah PWA yang ditemukan saat audit kontras. Ringkasannya:

**B1 — Aset dan budget.** Ketujuh `*-v2.webp` di-downscale ke lebar 800px dengan kualitas dipertahankan (WebP q=92) lewat `scripts/optimize_illustrations.py` (idempoten). Prescache turun dari ±975 KB menjadi di bawah 700 KB. Master PNG di `design/illustrations/v2/` tidak diubah. Atribut `width`/`height` pada template disinkronkan ke ukuran baru. Matriks aset (aktif / superseded / pending-v2) ada di `app/static/illustrations/README.md`. Lima SVG v1 yang yatim disimpan dan ditandai superseded, tidak dihapus. Cache Service Worker naik ke `presensi-shell-v9`.

**B2 — Ketahanan worker.** Precache berganti dari `cache.addAll` (gagal atomik: satu aset hilang membuang seluruh shell offline) menjadi `Promise.allSettled` per-aset dengan peringatan di console untuk aset yang gagal. Ditambah `isPrivatePath` sebagai pertahanan berlapis: `/attendance`, `/face`, `/admin`, `/student`, `/teacher`, `/profile`, `/auth`, `/evidence`, dan `/logout` tidak pernah dilayani dari cache.

**B3 — Manifest dan ikon.** Manifest kini mendeklarasikan `id`, `lang: "id"`, `dir: "ltr"`, dan `orientation: "portrait-primary"`. `apple-touch-icon` 180×180 dibuat dari icon-192 dan diberi `sizes="180x180"`; ditambahkan `apple-mobile-web-app-status-bar-style` dan `apple-mobile-web-app-title`. Ikon yang ada sudah memiliki safe zone, hanya diverifikasi.

**B4 — Install dan update (role Siswa saja).** Banner install muncul lewat `beforeinstallprompt`, gated ke `body.student-shell` (Guru/Admin tidak pernah melihatnya), bisa "Nanti" (disimpan di localStorage), dan otomatis disembunyikan saat kamera enrollment/check-in aktif. Toast "Versi baru tersedia" muncul saat Service Worker menunggu aktivasi. Kegagalan registrasi worker kini dilog, bukan ditelan diam-diam.

**B5 — Penjaga offline presensi.** `attendance-checkin.js` menonaktifkan CTA dan memberi pesan eksplisit saat `navigator.onLine === false`, изменения status saat koneksi kembali, dan menolak membuka kamera serta membaca lokasi saat offline. Tidak ada antrean offline: kebijakan "presensi wajib online" ditegakkan client-side.

**B6 — Empty state.** `empty.svg` dipasangkan ke 10 empty state Admin (`analytics` ×2, `analytics_run`, `attendance`, `audit_logs`, `class_detail`, `class_list`, `schedule_exception_list`, `teacher_list`, `academic_year_list`, `schedule_list`) dan 1 Guru (`dashboard`). Sengaja dilewati: `preview-empty` (panel preview inline) dan blok `role="alert"` (error) yang diberi ilustrasi akan menyesatkan.

**B7 — Audit visual.** `scripts/visual_audit.py` merender route publik dan route terautentikasi ketiga role (fixture sintetis via test client) lalu mengambil screenshot headless Chrome dan mengukur overflow horizontal. Chrome di Windows meng-clamp lebar layout ke ~504px sehingga `--window-size` tidak bisa dipakai langsung; halaman dirender di dalam iframe berukuran tetap agar media query benar-benar melihat lebar ponsel. Hasil: **nol page-level horizontal overflow** di Siswa 360px, Guru 768px, Admin 1440px, serta pass zoom 200% (640/720px). Screenshot tersimpan di `docs/visual-audit/`.

**B8 — Polish responsif.** Karena audit overflow bersih, tidak diperlukan perbaikan responsif tambahan pada tahap ini.

Verifikasi: suite penuh **282 tes** (dari 274, +8 tes regresi PWA baru), `npm run build:css`, `node --check` untuk Service Worker, `pwa-register.js`, dan `attendance-checkin.js`.

Skrip yang dapat dijalankan ulang:
...
Yang **tidak** diverifikasi pada tahap ini dan tetap menjadi T32: kamera, GPS, pose, blink, pencahayaan, dan koneksi lambat/putus pada perangkat Android nyata melalui HTTPS. Audit ini hanya render sintetis.
